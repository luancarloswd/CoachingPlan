...
{
    class ApplicationCookieIdentityValidator
    {
        private static async Task<bool> VerifySecurityStampAsync(ApplicationUserManager manager, Usuario user, CookieValidateIdentityContext context)
        {
        ...
        private static Task<bool> VerifyClientIdAsync(ApplicationUserManager manager, Usuario user, CookieValidateIdentityContext context)
        {
            string clientId = context.Identity.FindFirstValue("AspNet.Identity.ClientId");
            if (!string.IsNullOrEmpty(clientId) && user.Dispositivo.Any(c => c.Id.ToString() == clientId))
            {
        ...
        public static Func<CookieValidateIdentityContext, Task> OnValidateIdentity(TimeSpan validateInterval, Func<ApplicationUserManager, Usuario, Task<ClaimsIdentity>> regenerateIdentity)
        {
